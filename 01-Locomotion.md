...
:height: 300px
:name: Fig1-3
:alt: Force diagram, with the tension, $T$, broken down into its $x$ and $y$ components.

Force diagram, with the tension, $T$, broken down into its $x$ and $y$ components.
``` 
...
```
Note that the acceleration is not constant, that is, it varies with the angle $\theta$, which is dependent on the position $x$ of the foot along the horizontal. Hence we cannot use simple approaches like the kinematic equations to solve for the time it takes to complete a step.  We will approach the solution to Eqn. {eq}'max_2' differently than what is done in an introductory physics course. 
